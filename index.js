...
bot.onText(/\/list/, async (msg) => {
  ...
  for (const t of tokens) {
    ...
    try {
      ...
      const currentMult = currentMC > 0 ? (currentMC / t.baselineMC).toFixed(2) : "?";
      const progress = currentMC > 0
        ? Math.min(((currentMC / t.baselineMC - 1) / (t.multiplier - 1)) * 100, 100).toFixed(0)
        : "?";
      ...
      text =
        ...
        `📈 Current:  ${formatMC(currentMC)} (${currentMult}x)\n` +
        `🎯 Target: ${t.multiplier}x | Progress: ${progress}%`;
    } catch {
      text =
        `${status} *${t.name}*\n` +
        `\`${t.mint.slice(0, 8)}...${t.mint.slice(-6)}\`\n\n` +
